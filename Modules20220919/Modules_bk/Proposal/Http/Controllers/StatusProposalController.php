...
class StatusProposalController extends Controller
{
    ...
    function index($id = NULL){
        if ($id) {
            ...
            $banks          = ComBank::all();
            $disposisi      = PublicTrxProposalTimeline::where('trx_proposal_id',$id)->get();
            $statusDisposisi = PublicTrxProposalTimeline::getStatusDisposisi($id);
            ...
            return view('proposal::' . $this->folder_path . '.' . $filename_page, compact('title', 'proposal', 'pemohon','userPemohon','banks','disposisi','statusDisposisi'));
        }else{
            $filename_page 	= 'index-status';
            $title 			= 'Status Proposal';
