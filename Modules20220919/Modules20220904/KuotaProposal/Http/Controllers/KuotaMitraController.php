...
class KuotaMitraController extends Controller
{
    private $folder_path = '';
    private $kuotaTp     = 'mitra';
    ...
    function getData(Request $request){
        $data = PublicTrxKuotaProposal::leftJoin('public.trx_mitra_kemaslahatan as mitra',DB::Raw('mitra.trx_mitra_kemaslahatan_id::varchar'),'trx_kuota_proposal.kuota_cd')
                ->select(
                    "trx_kuota_proposal_id",
                    "trx_year",
                    "kuota_cd",
                    DB::Raw("mitra.mitra_kemaslahatan_nm as kuota_nm"),
                    "kuota"
                )
                ->where(function ($query) use($request)
                {
                    ...
                    if ($request->nama) {
                        $query->orWhere("mitra.mitra_kemaslahatan_nm", "ILIKE" ,'%'.$request->nama.'%');
                    }
