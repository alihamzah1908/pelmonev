...
<div id="accordion-default">
    ...
    <div class="card">
        ...
        <div class="card-body">                            
			...
			<!--<div class="col-md-12">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Mitra Kemaslahatan<span class="text-danger">*</span></label>
                                <textarea name="mitra_kemaslahatan" id="mitra_kemaslahatan" class="form-control" required="required">{{ $analisisResiko ? $analisisResiko->analisa_legalitas : '' }}</textarea>
                            </div>
                        </div>
			-->
                        <div class="col-md-12">
                            <div class="form-group form-group-float">
                                ...
                                <textarea name="resiko_reputasi" id="analisa_legalitas" class="form-control" required="required">{{ $analisisResiko ? $analisisResiko->resiko_reputasi : '' }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Resiko Keberlanjutan<span class="text-danger">*</span></label>
                                <textarea name="resiko_keberlanjutan" id="analisa_peraturan" class="form-control" required="required">{{ $analisisResiko ? $analisisResiko->resiko_keberlanjutan : '' }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Mitigasi Resiko<span class="text-danger">*</span></label>
                                <textarea name="mitigasi_resiko" id="analisa_hukum" class="form-control" required="required">{{ $analisisResiko ? $analisisResiko->mitigasi_resiko_reputasi : '' }}</textarea>
                            </div>
                        </div>
			<div class="col-md-12">
                            <div class="form-group form-group-float">
                                <label class="form-group-float-label is-visible">Mitigasi Resiko Keberlanjutan<span class="text-danger">*</span></label>
                                <textarea name="mitigasi_resiko_keberlanjutan" id="analisa_hukum" class="form-control" required="required">{{ $analisisResiko? $analisisResiko->mitigasi_resiko_keberlanjutan : '' }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group form-group-float">
                                ...
                                <textarea name="kesimpulan" id="kesimpulan" class="form-control" required="required">{{ $analisisResiko ? $analisisResiko->kesimpulan : '' }}</textarea>
                            </div>
                        </div>
                    </div>
                    @if (isRoleUser('bidmr'))    
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="tab-penprog-deskripsi">
                        <form class="form-validate-jquery" id="form-deskripsi" action="#">
					      ...
					      <option value="dummy1" @if($deskripsi) {{ $deskripsi->sub_program == 'dummy1' ? ' selected' : ''}} @endif>Sub Program Dummy 1</option>
					      <option value="dummy2" @if($deskripsi) {{ $deskripsi->sub_program == 'dummy2' ? ' selected' : ''}} @endif>Sub Program Dummy 2</option>
					      <option value="dummy3" @if($deskripsi) {{ $deskripsi->sub_program == 'dummy3' ? ' selected' : ''}} @endif>Sub Program Dummy 3</option>
					</select>
				    </div>
                                </div>
                            </div>
   ...
   <div class="card">
	<div class="card-body">
	  	@if(isJabatanUser([9, 7, 5, 4, 3, 11, 12, 15, 17, 18, 19]))
		<div class="row">
		       <div class="d-flex justify-content-start align-items-center">
			   <button id="rekomendasi-manaj" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id }}' data-status='{{ $proposal->proses_st }}'>Rekomendasikan</button>
			   <button id="rekomendasi-manaj-back" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id }}' data-status='{{ $proposal->proses_st }}'>Kembalikan</button>
		       </div>
		 </div>
	  	@elseif(isJabatanUser(13) && isRoleUser(['bidmr']))
		@php
                $nominal = $proposal->nominal;
		@endphp
		<button id="disposisi-proposal-manaj-resiko" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{$proposal->trx_proposal_child_id}}' data-status='{{ $proposal->proses_st }}' data-nominal='{{$nominal}}'>Rekomendasikan</button>
		<button id="rekomendasi-manaj-back" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id }}' data-status='{{ $proposal->proses_st }}'>Kembalikan</button>
		@else 
...
<script>
    ...
    $(document).ready(function(){
		...
		e.preventDefault();
                ...
                var record  = new FormData(this);
	        var url     = '/proposal/simpan/analis-resiko/'
                var method  = 'POST';
                swal({
                    title               : 'Simpan data ?',
                    type                : "question",
    ...
    function resetPengurusKerjasama(id) {
     ...
     $(document).on('click', '#rekomendasi-manaj',function(){
	...
	$(document).on('click', '#rekomendasi-manaj-back',function(){
            var trx_proposal_mitra_id = $(this).attr('data-bind')
            var status = $(this).attr('data-status')
            swal({
	...
	$(document).on('click', '#disposisi-proposal-manaj-resiko',function(){
            var trx_proposal_mitra_id = $(this).attr('data-bind')
            var status = $(this).attr('data-status')
	    var nominal = $(this).attr('data-nominal')
	    if(parseInt(nominal) < parseInt(500000000)){
		var text_title = 'Apakah anda akan melanjutkan ke Rapat BP?'
		var last_status = 'PROSES_RAPAT_BP'
	    }else{
		var text_title = 'Apakah anda akan melanjutkan ke Rapat KEK?'
		var last_status = 'PROSES_RAPAT_KEK'
	    }
            swal({
                title             : text_title,
                type              : "question",
                        ...
                        success: function(response)
                        {
                            if (response.status == 'ok') {
                                swal({
                                    ...
                                    //reset('')
                                    swal.close();
                                    window.location.href = '/proposal/list-proposal';
                                });
                            }else{
                                swal({title: "Proses gagal",type: "error",showCancelButton: false,showConfirmButton: false,timer: 1000});
                            }
