...
{
    private $folder_path    = 'proposal-monitoring';
    private $prosesSt       = 'PROSES_44';
    ...
    function index($id = NULL){
        if ($id) {
            ...
            $proposal       = PublicTrxProposal::join('trx_proses_status','trx_proses_status.trx_proses_status_id','trx_proposal.proses_st')
                                ->join('trx_mitra_kemaslahatan','trx_mitra_kemaslahatan.trx_mitra_kemaslahatan_id', 'trx_proposal.trx_mitra_kemaslahatan_id')
                                ->find($id);
