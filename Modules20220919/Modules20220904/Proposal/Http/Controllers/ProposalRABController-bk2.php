...
class ProposalController extends Controller
{
    ...
    function getDataMitra(Request $request)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('actions', function ($data) {
                ...
                if (isRoleUser('pemohon') && $data->proses_st == 'PROSES_CPM') {
                    $actions .= '<div class="d-flex justify-content-center">
                            <a href="' . url('proposal') . "/list-proposal/mitra/" . $data->trx_proposal_mitra_id . '/show" class="btn btn-sm btn-info mx-1 px-2"><i class="icon icon-search4"></i></a>
                    ...
                    $actions .= '<div class="d-flex justify-content-center">
                            ...
                            <button id="disposisi-akr-back" class="btn btn-sm btn-danger mx-1 px-2" data-bind=' . $data->trx_proposal_child_id . '>Kembalikan</button>
                            </div>';
                } else {
                    $actions .= '<div class="d-flex justify-content-center">
                            <a href="' . url('proposal') . "/list-proposal/mitra/" . $data->trx_proposal_mitra_id . '/show" class="btn btn-sm btn-info mx-1 px-2"><i class="icon icon-search4"></i></a>
    ...
    public function detail_mitra_proposal($id, Request $request)
    {
        ...
        $pelaksanaanPenilaian = PublicTrxProposalLayakTeknisPelaksanaanPenilaian::where('trx_proposal_id', $id)->first();
        $filename_page = 'detail-mitra';
        return view('proposal::' . $this->folder_path . '.' . $filename_page, compact('proposal', 'title', 'deskripsi', 'mitra', 'analisa', 'pelaksanaanPenilaian'));
    }
    ...
    public function send_proposal_mitra($id, Request $request)
    {
        ...
        // dd($pemohon);
        if ($pemohon) {
            //$proposal = \App\Models\PublicTrxProposalMitra::find($pemohon->trx_proposal_mitra_id);
            ...
            if ($request["status"] == 'kembalikan') {
                $proposalmitra->proses_st = $request["proses"];
                $proposal->proses_st = $request["proses"];
            } else {
