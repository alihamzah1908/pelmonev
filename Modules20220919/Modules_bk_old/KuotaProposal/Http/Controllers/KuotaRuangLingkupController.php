...
class KuotaRuangLingkupController extends Controller
{
    private $folder_path = '';
    private $kuotaTp     = 'ruanglingkup';
    ...
    function getData(Request $request){
        $data = PublicTrxKuotaProposal::leftJoin('public.com_code as rl','rl.com_cd','trx_kuota_proposal.kuota_cd')
                ->select(
                    "trx_kuota_proposal_id",
                    "trx_year",
                    "trx_kuota_proposal.kuota_cd",
                    DB::Raw("rl.code_nm as kuota_nm"),
                    "kuota"
                )
                ->where(function ($query) use($request)
                {
                    ...
                    if ($request->nama) {
                        $query->where("rl.code_nm", "ILIKE" ,'%'.$request->nama.'%');
                    }
