...
class ProgramController extends Controller
{
    ...
    function history($id = NULL, Request $request)
    {
        $baseUrl = 'proposal';
        $currUserData = AuthUser::mGetDetailUser(Auth::user()->user_id)->first();
        if ($id) {
            ...
            $filename_page     = 'pelmonev.program.history';
            $title             = 'History Pelmonev';
            \LogActivity::saveLog(
                $logTp = 'visit',
                $logNm = "Membuka Menu $title"
            );
    ...
    public function data_program(Request $request)
    {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('proses_st_nm', function ($data) {
                return "<span class='badge badge-info d-block'>Kegiatan Kemaslahatan</span>";
            })
    ...
    public function data_pencairanperikatan(Request $request)
    {
        ...
        if ($request["type"] == 'perikatan') {
            return DataTables::of($data)
                ...
                ->addColumn('proses_st_nm', function ($data) {
                    return "<span class='badge badge-info d-block'>Perikatan-DraftPKS</span>";
                })
                ->addColumn('nominal', function ($data) {
                    $id = $data->trx_proposal_child_id != '' ? $data->trx_proposal_child_id : $data->trx_proposal_mitra_id;
                    $total = \App\Models\PublicTrxProposalRAB::select(DB::raw('sum(total_bpkh) as nominal_rekomendasi'))->where('trx_proposal_id', $data->trx_proposal_child_id)->first();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('proses_st_nm', function ($data) {
                    return "<span class='badge badge-info d-block'>pencairan-draftPinBuk</span>";
                })
                ->addColumn('nominal', function ($data) {
                    $id = $data->trx_proposal_child_id != '' ? $data->trx_proposal_child_id : $data->trx_proposal_mitra_id;
                    $total = \App\Models\PublicTrxProposalRAB::select(DB::raw('sum(total_bpkh) as nominal_rekomendasi'))->where('trx_proposal_id', $data->trx_proposal_child_id)->first();
    ...
    public function detailPerikatan($id, Request $request)
    {
        ...
        $userPemohon    = AuthUser::where('default_key', $proposal->trx_pemohon_id)->first();
        $banks          = ComBank::all();
        $proposalMitraUpdated  = PublicTrxProposalMitra::where('trx_proposal_child_id', $proposal->trx_proposal_id)->first();
        $disposisi      = PublicTrxProposalTimeline::join('trx_proses_status', 'trx_proses_status.trx_proses_status_id', 'trx_proposal_timeline.status')->where('trx_proposal_id', $id)
            ->select(
                "trx_proposal_timeline_id",
                "trx_proposal_id",
    ...
    public function disposisi($id, Request $request)
    {
        ...
        if ($request["proses"] == 'PROSES_TERIMA') {
            ...
            $proposalMitraUpdate->trx_notes_disposisi = $request["notes"];
        }
        $proposal->save();
    ...
    {
        // dd($request["lampiran_photo_rab"]);
        $term1 = count(array_filter($request["termin_1"])) == count($request["termin_1"]);
        $term2 = count(array_filter($request["termin_2"])) == count($request["termin_2"]);
        $term3 = count(array_filter($request["termin_3"])) == count($request["termin_3"]);
        ...
        $data->sdgs = json_encode($request["sdgs"]);
        if ($term1 == true) {
            $data->termin = json_encode($request["termin_1"]);
        } else if ($term2 == true) {
            $data->termin = json_encode($request["termin_2"]);
        } else if ($term3 == true) {
            $data->termin = json_encode($request["termin_3"]);
        }
        ...
        if ($request->hasFile('lampiran_rab')) {
            $file = $request->file('lampiran_rab');
            $lampiran_rab = Uuid::uuid4() . "." . $file->getClientOriginalExtension();
            $image['filePath'] = $lampiran_rab;
            $file->move(storage_path('app/public/pelmonev/pks'), $lampiran_rab);
            $data->lampiran_rab = $lampiran_rab;
        }
        if ($request->hasFile('lampiran_lainya')) {
            $file = $request->file('lampiran_lainya');
            $lampiran_lainya = Uuid::uuid4() . "." . $file->getClientOriginalExtension();
            $image['filePath'] = $lampiran_lainya;
            $file->move(storage_path('app/public/pelmonev/pks'), $lampiran_lainya);
            $data->lampiran_lainya = $lampiran_lainya;
        }
        if ($request->hasFile('lampiran_photo_rab')) {
            $file = $request->file('lampiran_photo_rab');
            $lampiran_photo = array();
            foreach ($file as $key => $val) {
                $lampiran_photo[$key] = Uuid::uuid4() . "." . $val->getClientOriginalExtension();
                $image['filePath'] = $lampiran_photo[$key];
                $val->move(storage_path('app/public/pelmonev/pks'), $lampiran_photo[$key]);
            }
            $data->lampiran_photo = json_encode($lampiran_photo);
        }
        $data->save();
        if ($request["pasal"]) {
            if ($request["trx_pasal_id"]) {
                foreach ($request["trx_pasal_id"] as $key => $val) {
                    $check = \App\Models\TrxPasalPelmonev::find($val);
                    if ($check->count() == count($request["ayat_tambahan"])) {
                        $pasal = \App\Models\TrxPasalPelmonev::find($request["trx_pasal_id"][$key]);
                    } else {
                        $pasal = new \App\Models\TrxPasalPelmonev();
                    }
                    $pasal->trx_pelmonev_pks_id = $data->trx_pks_id;
                    $pasal->pasal = $request["pasal"][$key];
                    $pasal->ayat_tambahan = $request["ayat_tambahan"][$key];
                    $pasal->save();
                }
            } else {
                foreach ($request["ayat_tambahan"] as $key => $val) {
                    $pasal = new \App\Models\TrxPasalPelmonev();
                    $pasal->trx_pelmonev_pks_id = $data->trx_pks_id;
                    $pasal->pasal = $request["pasal"][$key];
                    $pasal->ayat_tambahan = $request["ayat_tambahan"][$key];
    ...
    function spjtm($id)
    {
        ...
        if ($data) {
            ...
            $view       = view('proposal::pelmonev.print.spjtm', compact('data'))->render();
            // return $view;
            $pdf        = PDF::loadHtml($view);
            $pdf->setOptions(['isHtml5ParserEnabled' => true, 'setIsRemoteEnabled' => true]);
    ...
    public function download_lampiran_file($file)
    {
        $files = storage_path("app/public/pelmonev/pks/" . $file);
        $headers = array(
        ...
        return Response::download($files, 'file-lampiran', $headers);
    }
}
