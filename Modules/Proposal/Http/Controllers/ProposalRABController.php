...
class ProposalRABController extends Controller
{
    ...
    function getData(Request $request){
        $datax = PublicTrxProposalRAB::leftJoin('com_code as freq','freq.com_cd','trx_proposal_rab.satuan')
                ->select(
                    "trx_proposal_rab_id",
		    ...
		    "total_bpkh",
		    "biaya_satuan_bpkh",
		    "trx_proposal_rab.kategori as jenis_rab",
                );
		if($request["kategori"]){
		   $datax->join('trx_proposal','trx_proposal.trx_proposal_id','trx_proposal_rab.trx_proposal_id');
                   $datax->where('trx_proposal_rab.trx_proposal_id',$request->id);
		   $datax->where('kategori', $request->kategori);
		}else{
		   $datax->join('trx_proposal','trx_proposal.trx_proposal_id','trx_proposal_rab.trx_proposal_id');
                   $datax->where('trx_proposal_rab.trx_proposal_id',$request->id);
		}
	...
	->addColumn('action_2', function($data){
        ...
        ->editColumn('biaya_satuan', function($data){
            return (int)$data->biaya_satuan_mitra ? (int)$data->biaya_satuan_mitra : (int)$data->biaya_satuan;
        })
	...
	->editColumn('jumlah_unit', function($data){
        ...
        ->editColumn('biaya_satuan_mitra', function($data){
            return $data->biaya_satuan_mitra ? (int)$data->biaya_satuan_mitra : (int)$data->biaya_satuan;
        })
	->addColumn('total_bpkh', function($data){
	    if (isRoleUser(['regas'])) {
                 return '<input type="text" name="biaya_rekomendasi" class="biaya-rekomendasi-bpkh rekomendasi-bpkh-'.$data->trx_proposal_rab_id.' form-control form-child-field" placholder="Isi biaya rekomendasi BPKH" value="Rp. '. number_format($data->total_bpkh,2,',','.') .'">';
            }else{
	         return $data->total_bpkh ? (int)$data->total_bpkh : NULL;
	    }
	})
	->addColumn('jumlah_unit_bpkh', function($data){
	    if (isRoleUser(['regas'])) {
                 return '<input type="text" name="jumlah_unit_bpkh" class="jumlah-unit-bpkh form-control form-child-field" placholder="Isi biaya rekomendasi BPKH" value="' . $data->jumlah_unit_bpkh .'">';
            }else{
	         return $data->jumlah_unit_bpkh ? (int)$data->jumlah_unit_bpkh : NULL;
	    }
	})

        ->addColumn('biaya_satuan_bpkh', function($data){
	  if (isRoleUser(['regas'])) {
		$unit = (int)$data->jumlah_unit != '' ? $data->jumlah_unit : $data->jumlah_unit_mitra ;
            	return '<input type="hidden" name="proposal_id" class="proposal_id" value=' . $data->trx_proposal_rab_id .'>
			<input type="hidden" name="jumlah_unit" class="jumlah_unit" value=' . $unit .'>
		 	<input type="text" name="satuan_rekomendasi" class="biaya-satuan-rekomendasi-bpkh form-control form-child-field" placholder="Isi biaya satuan rekomendasi BPKH" value="Rp. '. number_format($data->biaya_satuan_bpkh,2,',','.') .'">';
          }else{
		return $data->biaya_satuan_bpkh ? (int)$data->biaya_satuan_bpkh : NULL;
          }
	})
        ->rawColumns(['actions','total_bpkh','biaya_satuan_bpkh','action_2','jumlah_unit','jumlah_unit_1','jumlah_unit_bpkh'])
	...
	->editColumn('jumlah_unit', function($data){
        ...
        ->editColumn('total', function($data){
            return (int)$data->total_mitra != '' ? $data->total_mitra : $data->total;
        })
        ->editColumn('biaya_satuan_mitra', function($data){
            return $data->biaya_satuan_mitra ? (int)$data->biaya_satuan_mitra : (int)$data->biaya_satuan;
        })
	...
	//dd($request->all());
		...
		$rab->kategori = $request->kategori_rab;
                ...
                switch (roleUser()) {
                    case 'mitra':
                        ...
                        if($request->has('rab_jumlah_unit') && $request->has('rab_satuan') || $request->has('rab_biaya_satuan')) $rab->total_mitra = $request->rab_jumlah_unit * uang($request->rab_biaya_satuan);
			...
			break;
                    case 'regas':
                        ...
                        if($request->has('rab_jumlah_unit') && $request->has('rab_satuan'))
			$rab->total_bpkh = $request->rab_jumlah_unit * uang($request->rab_biaya_satuan);
                        if($request->has('biaya_rekomendasi')) $rab->total_bpkh = uang($request->biaya_rekomendasi);
			if($request->has('satuan_rekomendasi')) $rab->biaya_satuan_bpkh = uang($request->satuan_rekomendasi);
			break;
                    default:
	...
	//dd(uang($request->satuan_rekomendasi));
        try {
            DB::beginTransaction();
                ...
                switch (roleUser()) {
                    case 'mitra':
                        ...
                        if($request->has('jumlah_unit_bpkh')) $rab->jumlah_unit_bpkh = $request->jumlah_unit_bpkh;
			if($request->has('biaya_rekomendasi')) $rab->total_bpkh = uang($request->biaya_rekomendasi);
			if($request->has('satuan_rekomendasi')) $rab->biaya_satuan_bpkh = uang($request->satuan_rekomendasi);
			break;
                    case 'regas':
