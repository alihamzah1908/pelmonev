...
class PrintController extends Controller
{
    ...
    function sk($id)
    {
        $data       = PublicTrxProposal::join('trx_proses_status', 'trx_proses_status.trx_proses_status_id', 'trx_proposal.proses_st')
            ...
            ->find($id);
        $proposalMitraUpdated  = PublicTrxProposal::where('trx_proposal_id', $id)->first();
        $pelaksanaanPenilaian = PublicTrxProposalLayakTeknisPelaksanaanPenilaian::where('trx_proposal_id', $data->trx_proposal_id)->first();
        if ($data) {
            $resultName = date('Ymd_His') . "_SK_$id.pdf";
            $view       = view('proposal::print.sk.pelayanan-haji', compact('proposalMitraUpdated', 'data', 'pelaksanaanPenilaian'))->render();
            // return $view;
    ...
    function ringkasan($id)
    {
        $data       = PublicTrxProposal::join('trx_proses_status', 'trx_proses_status.trx_proses_status_id', 'trx_proposal.proses_st')
            ...
            ->find($id);
        $pelaksanaanPenilaian = PublicTrxProposalLayakTeknisPelaksanaanPenilaian::where('trx_proposal_id', $data->trx_proposal_id)->first();
        $deskripsi      = PublicTrxProposalLayakTeknisDeskripsi::where('trx_proposal_id', $data->trx_proposal_id)->first();
        if ($data) {
            $resultName = date('Ymd_His') . "_SK_$id.pdf";

            $view       = view('proposal::print.ringkasan.ringkasan', compact('data', 'pelaksanaanPenilaian', 'deskripsi'))->render();
            // return $view;
            $pdf        = PDF::loadHtml($view);
            $pdf->setOptions(['isHtml5ParserEnabled' => true, 'setIsRemoteEnabled' => true]);
            $pdf->setPaper('Legal', 'potrait');
