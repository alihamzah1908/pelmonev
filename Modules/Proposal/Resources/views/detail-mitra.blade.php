...
<div id="accordion-default">
    <div class="card">
        ...
        <div id="accordion-detail" class="collapse show" data-parent="#accordion-default">
		    ...
		    <!-- <li class="nav-item"><a href="#tab-data-rab-analisa" class="nav-link rounded-left" data-toggle="tab"><i class="{{ $proposal->rab_fill_st == 1 ? 'icon-floppy-disk' : 'icon-list' }} d-block mb-1 mt-1"></i>Analisa Rencana Anggaran Biaya </a></li>--!>
                ...
                <div class="tab-content">
					...
					@endphp
						...
						@foreach($select as $val)	
						    <option value="{{ $val->code_nm}}"{{ $val->com_cd == $deskripsi->sub_program ? ' selected' : '' }}>{{$val->code_nm}}</option>
						@endforeach
					     @else 
						@foreach($select as $val)	
						   <option value="{{ $val->code_nm}}">{{$val->code_nm}}</option>
						@endforeach
					     @endif					
					</select>
				    </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group form-group-float">
                                        ...
                                        <input type="text" name="rencana_lokasi" class="form-control" placeholder="Rencana Lokasi" aria-invalid="false" required="required" value="{{ $proposal ? getRegionNm($proposal->region_prop) . ', ' . getRegionNm($proposal->region_kab) . ', ' .  getRegionNm($proposal->region_kec)  : '' }}">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group form-group-float">
                                        ...
                                        <input type="text" name="penerima_maslahat" class="form-control" placeholder="Penerima Maslahat" aria-invalid="false" required="required" value="{{  $pemohon ? $pemohon->pemohon_nm : '' }}">
                                    </div>
                                </div>
                            </div>
                            @if (isRoleUser('regas'))    
                    ...
                    <div class="tab-pane fade show" id="tab-penprog-pelaksanaan-penilaian">
                        <form class="form-validate-jquery" id="form-pelaksanaan-penilaian" action="#">
                            ...
                            <div class="form-group form-group-float">
                                ...
                                <textarea name="penilaian_lokasi" id="pelaksanaan_penilaian_lokasi" class="form-control">{{ $pelaksanaanPenilaian ? $pelaksanaanPenilaian->lokasi : '' }}</textarea>
                            </div>
                            <div class="form-group form-group-float">
    ...
    <div class="card">
	<div class="card-body">
	      <button id="disposisi-proposal-akr" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id }}' data-status='{{ $proposal->proses_st }}'>Rekomendasikan</button>
              <button id="disposisi-akr-back" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id }}'>Kembalikan</button>
	</div>
    </div>
    @else 
    <div class="card">
	<div class="card-body">
		<button id="disposisi-proposal-akr" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{ $proposal->trx_proposal_child_id}}' data-status='{{ $proposal->proses_st}}'>
		{{ isJabatanUser(6) && $proposal->proses_st == 'PROSES_AR' ? 'Lanjutkan ke Kadiv' : 'Rekomendasikan' }}
		</button>
	</div>
    </div>	
    @endif
...
$(document).on('click', '#disposisi-proposal-akr',function(){
    ...
    $('body').on('click', '.simpan-penilaian', function(e){
	 if (e.isDefaultPrevented()) {
			//--Handle the invalid form
            } else {
		e.preventDefault();
                swal({
                    title               : 'Simpan Aspek Penilaian?',
                    type                : "question",
                    showCancelButton    : true,
                    confirmButtonColor  : "#00a65a",
                    confirmButtonText   : "OK",
