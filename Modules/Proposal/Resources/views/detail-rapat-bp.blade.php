    ...
    <div class="card">
        ...
        <div id="klaisifikasi-program" class="collapse" data-parent="#accordion-default">
			...
			<div class="row">
		   	...
		   	<div class="row" id="klasifikasi_catatan" style="display:@if($klasifikasi) {{ $klasifikasi->tipe_klasifikasi == 'lengkap_dengan_catatan' ? ' ' : ' none' }} @else none @endif">
                            <div class="col-md-12">
                                <div class="form-group form-group-float">
    ...
    <div class="card">
        ...
        <div class="card-body">                            
			...
			<table class="table" width="50%">
				...
				<td>: </td>
				<td>{{ $proposal ? "Rp " . number_format($proposalmitra->nominal,2,',','.') : '' }}</td>
			     </tr>
			     <tr>
			     @php
...
<div class="card">
    ...
    <div class="row">
	<div class="col-md-12">
	@if(isRoleUser(['regas']) && $proposal->proses_st == 'PROSES_RBP')
	     <button id="print-ringkasan" class="btn btn-sm btn-primary mx-1 px-2">Print Ringkasan Proposal</button>
	     <button id="izinkan-rapat" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{$proposal->trx_proposal_id}}' data-status='{{$proposal->proses_st}}'>Izinkan</button>
	     <button id="kembalikan-rapat" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{ $proposal->trx_proposal_id}}' data-status='{{$proposal->proses_st}}'>Tolak</button>
	@elseif(isRoleUser(['regas']) && $proposal->proses_st == 'PROSES_KEK')
	     <button id="print-ringkasan" class="btn btn-sm btn-primary mx-1 px-2">Print Ringkasan Proposal</button>
	     <button id="izinkan-rapat" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{$proposal->trx_proposal_id}}' data-status='{{$proposal->proses_st}}'>Izinkan</button>
	     <button id="kembalikan-rapat" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{$proposal->trx_proposal_id}}' data-status='{{$proposal->proses_st}}'>Tolak</button>
	@elseif(isRoleUser(['regas']) && $proposal->proses_st == 'PROSES_TKEK')
	     <button id="rekomendasi-rapat" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{$proposal->trx_proposal_child_id}}' data-status='{{$proposal->proses_st}}'>Rekomendasikan</button>
             <button id="kembalikan-rapat" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{$proposal->trx_proposal_child_id}}' data-status='{{$proposal->proses_st}}'>Kembalikan</button>
	@elseif(isRoleUser(['regas']) && $proposal->proses_st == 'PROSES_TRBP')
	     <button id="rekomendasi-rapat" class="btn btn-sm btn-primary mx-1 px-2" data-bind='{{$proposal->trx_proposal_child_id}}' data-status='{{$proposal->proses_st}}'>Rekomendasikan</button>
             <button id="kembalikan-rapat" class="btn btn-sm btn-danger mx-1 px-2" data-bind='{{$proposal->trx_proposal_child_id}}' data-status='{{$proposal->proses_st}}'>Kembalikan</button>
	@endif
	</div>
...
$(document).on('click', '#disposisi-proposal-akr',function(){
    ...
    $(document).on('click', '#rekomendasi-rapat',function(){
            var trx_proposal_mitra_id = $(this).attr('data-bind')
            var status = $(this).attr('data-status')
	    console.log(status)
	    if(status == 'PROSES_TRBP'){
