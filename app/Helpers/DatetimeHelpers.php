...
if ( ! function_exists('tgl_indo')){
    function tgl_indo($tgl)
    {
        if ($tgl == NULL || $tgl == '0000-00-00 00:00:00') {
            ...
            $format     = date('Y-m-d', strtotime($ubah));
            $pecah      = explode("-",$format);   //memecah variabel berdasarkan -
            $hari       = $pecah[2];
            $bulan      = bulan($pecah[1]);
            $tahun      = $pecah[0];
            return $hari.' '.$bulan.' '.$tahun;//hasil akhir
        }
    }
}

//format tanggal yyyy-mm-dd
if ( ! function_exists('tgl_dan_hari_indo')){
    function tgl_dan_hari_indo($tgl)
    {
        if ($tgl == NULL || $tgl == '0000-00-00 00:00:00') {
            return "-";
        }else{
            $ubah       = gmdate($tgl, time()+60*60*8);
            $format     = date('Y-m-d', strtotime($ubah));
            $namaHari   = hari(date('N', strtotime($ubah)));

            $pecah      = explode("-",$format);   //memecah variabel berdasarkan -
            $hari       = $pecah[2];
            $bulan      = bulan($pecah[1]);
            $tahun      = $pecah[0];
            return $namaHari.', '.$hari.' '.$bulan.' '.$tahun;//hasil akhir
        }
    }
}
